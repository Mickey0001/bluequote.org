                ...
                <div id="themify_builder_lightbox_options_tab_items">
                        <?php foreach( $row_form_settings as $setting_key => $setting ): ?>
                            <li <?php if($setting_key==='setting'):?>class="current"<?php endif;?>>
                                <a href="#themify_builder_options_<?php  echo esc_attr( $setting_key ); ?>">
		...
		if($is_styling){
			...
			foreach ($class_fields as $field) {
				if (!empty($row['styling'][$field])) {
					if ('animation_effect' === $field && Themify_Builder_Model::is_animation_active()) {
						$row_classes[] = 'wow';
					}
					if ('hover_animation_effect' === $field && !Themify_Builder::$frontedit_active && Themify_Builder_Model::is_animation_active()) {
		...
		if ( ! $echo ) {
			...
			ob_start();
                ...
                if(!Themify_Builder::$frontedit_active){
                    ...
                    if ( ! empty( $row['column_alignment'] ) ) {
                        $row_content_classes[] = $row['column_alignment'];
                    }
                    ...
                    if ($is_styling && !empty($row['styling']['row_anchor'])) {
                        ...
                        $row_anchor = $row['styling']['row_anchor'];
                    }
                    if($count>0){
                ...
                if(!empty( $row_anchor )){
                    $row_attributes['data-anchor'] = $row_anchor;
                }
